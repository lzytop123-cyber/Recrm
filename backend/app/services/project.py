...
def create_project(db: Session, user: User, payload: ProjectCreate) -> Project:
    ...
    if payment_deferred:
        if approval_flow.select_rule(db, "project_no_contract", {}) is not None:
            approval_flow.start_instance(
                ...
                biz_id=project.id,
                initiator=user,
                title=approval_flow.approval_title('无合同立项' if not contract_id else '无到款立项', project.name),
    ...
    # AP-07 项目立项审批（合同项目）：部门负责人 → 中心负责人
        ...
        approval_flow.start_instance(
            ...
            biz_id=project.id,
            initiator=user,
            title=approval_flow.approval_title("项目立项", project.name),
            department_id=project.department_id,
...
def start_planning(db: Session, user: User, project_id: int) -> Project:
    ...
    if (
        ...
        approval_flow.start_instance(
            ...
            biz_id=project_id,
            initiator=user,
            title=approval_flow.approval_title("项目交接确认", project.name),
            department_id=project.department_id,
            deep_link=f"/projects/{project.id}",
            commit=True,
        )
...
def accept_project(
    ...
    if approval_flow.select_rule(db, "project_acceptance", {}) is not None:
        ...
        approval_flow.start_instance(
            ...
            biz_id=project.id,
            initiator=user,
            title=approval_flow.approval_title("项目验收", project.name),
...
def submit_finance_check(
    ...
    if approval_flow.select_rule(db, "project_settlement", {}) is not None:
        approval_flow.start_instance(
            ...
            biz_id=project.id,
            initiator=user,
            title=approval_flow.approval_title("结项归档核验", project.name),
            department_id=project.department_id,
            deep_link=f"/projects/{project.id}",
            commit=False,
        )
...
def terminate_project(
    ...
    if approval_flow.select_rule(db, "project_terminate", {}) is not None:
        approval_flow.start_instance(
            ...
            biz_id=project.id,
            initiator=user,
            title=approval_flow.approval_title("终止项目", project.name),
