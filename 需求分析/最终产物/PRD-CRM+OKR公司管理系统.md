...
文档性质：正式PRD，作为高保真原型、研发拆分、测试设计和实施准备的需求基线
...
|---|---|
| 文档类型 | 正式PRD |
| 需求范围 | CRM、OKR、经营管理、员工执行、交付协作、固定资产、线索池、AI知识库 |
| 系统需求确认 | 已完成 |
| PRD确认 | 已完成 |
| 功能基线 | 118项可验收功能需求 |
| 页面与路径基线 | 37个页面、12条核心路径 |
| 原型状态 | 高保真交互原型已同步更新固定资产管理与拍摄档期日历 |
...
7. 统一管理公司设备资产，重点保障新媒体拍摄器材可查、可借、可还、可盘、可维护并可追责。
...
| 销售管理 | 线索来源、管理层分配、跟进、转化、释放全链路留痕，重复客户可识别 |
...
| 资产利用 | 固定资产账实一致，拍摄器材借用、归还、维保与使用记录可追溯，档期冲突可提前识别 |
...
| 线索责任人 | 接收管理层分配的线索，跟进、转化或释放自己当前负责的线索 |
...
| 资产管理员 | 维护资产台账、分类、二维码、库存、盘点、维保、折旧与处置，审批领用归还事项 |
| 普通员工 | 查询可用设备，提交本人领用/借用与归还申请，使用移动端扫码并查看本人记录 |
| 系统管理员 | 配置角色权限、字典、流程、集成、审计和运行参数，不替代业务审批 |

### 3.2 核心使用场景

1. 全员录入线索，系统查重并记录来源；有效线索统一进入仅管理层可见的待分配池，由管理层勾选后批量分配给具体员工。
...
10. 资产管理员登记相机、镜头、灯具等设备并生成二维码；员工申请借用、扫码领用和归还，系统持续记录使用人、拍摄档期、设备状态与审批轨迹。
11. 新媒体部门通过拍摄档期日历安排拍摄任务，关联所需器材并在提交时校验人员与设备占用冲突。
...
- 固定资产登记入库、分类、二维码、领用归还审批、扫码盘点、维保提醒、库存预警、折旧、报废处置和报表分析。
- 新媒体拍摄档期周/月日历、拍摄任务、参与人员、场地说明、关联器材及人员/器材占用冲突校验。
...
- 历史客户、历史合同、历史员工档案迁移。
- 飞书邮件正文及附件采集入知识库。
- 专业财务固定资产总账、税务折旧申报、采购商城、供应商结算及仓储物流系统。
...
- 所有人均可录入线索；通过查重和基础校验的线索统一进入管理层待分配池，普通员工不可查看未分配线索及其客户信息。
- 管理层可勾选一条或多条待分配线索，选择一个或多个具体分配人并选择分配方式；未修改分配方式时默认在所选人员之间平均分配。
- 分配方式至少支持平均分配和逐条指定；系统提交前展示“线索—负责人”预览，并校验负责人有效性、重复客户及并发分配冲突。
- 每个客户只保留一个当前主负责人；批量分配中的单条失败不得伪装整批成功，必须返回成功、失败及失败原因明细。
- 每个人都可以释放自己当前被分配的线索；不得释放他人线索。释放后线索回到管理层待分配池，必须填写原因并保留完整历史。
- 录入、查重、分配、转派、释放、保护期变化、跟进、退回、转化和作废均记录操作者、时间、前后状态、原因及证据。
...
- 新媒体拍摄档期与讲师、主播、会议共用人员占用校验，同时关联固定资产占用；设备冲突未解决时不得确认档期。
- 拍摄档期变更或取消时同步更新关联器材的预占用状态，但不得删除既有审批、借用与归还记录。

### 5.5 固定资产与拍摄器材规则

- 每件资产使用内部唯一资产编号，实物二维码与系统台账一一对应；同一二维码不得绑定多件有效资产。
- 资产分类、品牌型号、序列号、所属部门、存放位置、购置日期、原值、责任人和当前状态为台账核心字段；相机、镜头、灯具等拍摄器材需补充关键规格。
- 普通员工只能申请本人使用或本人负责档期所需设备；资产管理员负责审批、出库确认、归还验收、盘点、维保、折旧与处置。
- 已预约、借出、维修、盘亏、报废或处置中的设备不得再次占用；审批时再次校验设备状态和时间区间，防止并发重复借用。
- 领用/借用、归还、转交、异常损坏、维保和盘点均保留操作者、时间、地点、设备状态、关联拍摄档期、附件及审批意见。
- 移动端扫码支持盘点、领用确认和归还验收；扫码结果必须显示资产身份与当前状态，不能仅以扫码成功替代业务确认。
- 折旧方法、使用年限、残值率和启用日期采用可配置规则；正式入账后变更规则须生成新版本，不覆盖历史计算结果。
- 报废处置必须先完成申请、审批和实物处置确认；处置后资产不可再借用，历史记录继续保留。
- 库存预警按分类可用数量或安全库存触发；维保提醒按计划日期、使用时长或使用次数触发，提醒不自动改变资产状态。
...
| 对象 | 核心状态 |
|---|---|
| 线索 | 草稿、待查重、待分配、已分配、保护中、跟进中、已释放、已转化、已作废 |
...
| 拍摄档期 | 草稿、待资源确认、待审批、已确认、执行中、已完成、已取消 |
| 固定资产 | 待入库、在库可用、已预占、借出中、待归还验收、维修中、盘亏、处置审批中、已报废、已处置 |
| 资产申请 | 草稿、待审批、已批准、已驳回、待领用、使用中、待归还验收、已归还、已取消、已逾期 |
...
#### 6.2.4 FR-012 配置线索分配规则
- 使用角色：董事会/经营管理层。
- 业务目的：配置管理层可用的线索分配方式和默认规则。
- 前置条件：用户已通过飞书身份进入系统，账号有效且具备对应功能与数据范围权限。
- 功能说明：在PAGE-006配置可用分配方式、默认平均分配、可选分配人员范围及失效人员处理；查看配置版本与历史记录；功能类型为配置管理功能。
...
#### 6.2.5 FR-013 配置线索池权限与分配原则
- 使用角色：董事会/经营管理层、系统管理员。
- 业务目的：确保未分配线索只对管理层可见，并控制可分配人员范围。
- 前置条件：用户已通过飞书身份进入系统，账号有效且具备对应功能与数据范围权限。
...
- 数据要求：读写或汇总客户、联系人、线索、线索跟进；记录来源、操作者、发生时间、当前状态与版本号，统计口径沿用数据字典。
- 权限要求：仅按岗位授权用户可在授权数据范围内操作；跨部门、财务、薪酬、绩效及敏感客户数据按最小权限和字段级脱敏控制。
- 验收条件：普通员工进入销售中心时不可见待分配池及未分配客户信息；管理层可查看和分配；规则调整形成授权范围且完整留痕。
- 来源追溯：用户故事：US-007；用例：UC-LEAD-004；业务过程：B1.1公司经营与执行闭环及对应子流程。

#### 6.2.6 FR-014 批量勾选待分配线索
- 使用角色：董事会/经营管理层。
- 业务目的：从管理层待分配池选择一条或多条线索进行统一分配。
- 前置条件：用户已通过飞书身份进入系统，账号有效且具备对应功能与数据范围权限。
- 功能说明：在PAGE-004、PATH-002按页勾选、全选或取消选择待分配线索，批量操作区实时展示已选数量；功能类型为用户交互功能。
- 规则约束：仅待分配状态可被选择；已被其他管理者分配或状态已变化的线索提交时必须阻止覆盖。
- 异常处理：显示规则不满足、状态冲突、数据缺失或无权限的明确原因；不得静默失败，不得因重试生成重复业务记录。
- 数据要求：读写或汇总客户、联系人、线索、线索跟进；记录来源、操作者、发生时间、当前状态与版本号，统计口径沿用数据字典。
- 权限要求：仅董事会/经营管理层可在授权数据范围内操作；普通员工不可见未分配线索。
- 验收条件：勾选一条或多条待分配线索后可进入批量分配；状态不允许或无权限时明确拒绝，不产生任何员工自行领取入口或记录。
- 来源追溯：用户故事：US-005；用例：UC-LEAD-005；业务过程：B1.1公司经营与执行闭环及对应子流程。

#### 6.2.7 FR-015 批量分配线索
- 使用角色：董事会/经营管理层。
- 业务目的：将勾选线索按选定方式分配给一个或多个具体员工。
- 前置条件：用户已通过飞书身份进入系统，账号有效且具备对应功能与数据范围权限。
- 功能说明：在PAGE-004选择具体分配人员和分配方式；分配方式未编辑时默认平均分配，并在提交前展示逐条分配预览；功能类型为用户交互功能。
- 规则约束：每条线索只能形成一个当前主负责人；支持平均分配和逐条指定，人员必须在职且具备线索承接权限。
- 异常处理：显示规则不满足、状态冲突、数据缺失或无权限的明确原因；不得静默失败，不得因重试生成重复业务记录。
- 数据要求：读写或汇总客户、联系人、线索、线索跟进；记录来源、操作者、发生时间、当前状态与版本号，统计口径沿用数据字典。
- 权限要求：仅按岗位授权用户可在授权数据范围内操作；跨部门、财务、薪酬、绩效及敏感客户数据按最小权限和字段级脱敏控制。
- 验收条件：选择人员但不修改方式时按平均分配生成预览；确认后逐条形成负责人和操作轨迹；部分失败返回成功/失败明细，不覆盖已生效分配。
- 来源追溯：用户故事：US-007；用例：UC-LEAD-006；业务过程：B1.1公司经营与执行闭环及对应子流程。

#### 6.2.8 FR-016 线索分配权限与并发校验
- 使用角色：董事会/经营管理层。
- 业务目的：在批量分配前校验管理权限、人员有效性、客户唯一负责人和线索当前状态。
- 前置条件：用户已通过飞书身份进入系统，账号有效且具备对应功能与数据范围权限。
- 功能说明：在PAGE-004、PATH-002提交批量分配时重新读取线索与人员状态，检测并发修改、重复客户和无效负责人；功能类型为系统自动功能。
- 规则约束：LEAD分配规则及客户唯一主负责人约束；状态变化必须满足当前业务对象生命周期。
- 异常处理：显示规则不满足、状态冲突、数据缺失或无权限的明确原因；不得静默失败，不得因重试生成重复业务记录。
- 数据要求：读写或汇总客户、联系人、线索、线索跟进；记录来源、操作者、发生时间、当前状态与版本号，统计口径沿用数据字典。
- 权限要求：仅按岗位授权用户可在授权数据范围内操作；跨部门、财务、薪酬、绩效及敏感客户数据按最小权限和字段级脱敏控制。
- 验收条件：提交分配；返回可执行的逐条预览或明确拒绝原因；确认后结果与责任轨迹可追溯，重复提交不产生重复分配。
...
- 数据要求：读写或汇总客户、联系人、线索、线索跟进；记录来源、操作者、发生时间、当前状态与版本号，统计口径沿用数据字典。
- 权限要求：仅线索责任人可在授权数据范围内操作；跨部门、财务、薪酬、绩效及敏感客户数据按最小权限和字段级脱敏控制。
- 验收条件：提交释放原因；线索回管理层待分配池；操作、状态变化和责任轨迹可追溯；成功时反馈业务结果和当前状态；失败、待处理、冲突或无权限时反馈原因与可采取动作。
...
- 数据要求：读写或汇总客户、联系人、线索、线索跟进；记录来源、操作者、发生时间、当前状态与版本号，统计口径沿用数据字典。
- 权限要求：仅按岗位授权用户可在授权数据范围内操作；跨部门、财务、薪酬、绩效及敏感客户数据按最小权限和字段级脱敏控制。
- 验收条件：命中退回规则；线索回管理层待分配池并留痕；操作、状态变化和责任轨迹可追溯；成功时反馈业务结果和当前状态；失败、待处理、冲突或无权限时反馈原因与可采取动作。
...
### 6.11 固定资产管理与新媒体拍摄资源协同

形成“设备入库—预约/领用审批—扫码出库—使用追踪—扫码归还—盘点维保—折旧处置—分析”的资产闭环，并把拍摄器材占用纳入新媒体拍摄档期。

| FR | 功能 | 主要角色 | 核心规则与可验收结果 |
|---|---|---|---|
| FR-101 | 设备登记入库 | 资产管理员 | 录入资产编号、分类、品牌型号、序列号、所属部门、位置、购置与价值信息；编号和有效二维码唯一，保存后形成待入库或在库资产。 |
| FR-102 | 资产分类与字段配置 | 资产管理员 | 维护相机、镜头、灯具、收音、稳定器、电脑等分类及分类专属规格；停用分类不影响历史资产。 |
| FR-103 | 生成与绑定资产二维码 | 资产管理员 | 单件或批量生成二维码，支持补打与换码；旧码失效留痕，不允许一码多资产。 |
| FR-104 | 查询资产台账与详情 | 资产管理员、普通员工 | 按编号、名称、分类、状态、部门、使用人和位置搜索；详情展示当前状态、完整使用记录、维修、盘点、折旧和处置轨迹。普通员工仅查看允许借用的信息。 |
| FR-105 | 提交领用/借用申请 | 普通员工 | 选择设备、用途、时间、关联项目或拍摄档期；提交时校验权限、设备状态和时间冲突，冲突时阻止提交并指出冲突记录。 |
| FR-106 | 审批领用/借用申请 | 资产管理员 | 批准、驳回或要求补充；批准前再次锁定并校验设备，审批意见和规则结果留痕。 |
| FR-107 | 移动端扫码领用 | 普通员工、资产管理员 | 扫码显示设备身份、申请人与当前状态；仅已批准申请可完成出库，成功后资产变为借出中并生成使用记录。 |
| FR-108 | 移动端扫码归还与验收 | 普通员工、资产管理员 | 员工扫码发起归还，管理员核验外观、附件和异常；验收通过恢复可用，异常则进入维修或责任处理。 |
| FR-109 | 借用逾期与使用记录追踪 | 资产管理员、普通员工 | 展示领用人、用途、关联档期、计划/实际时间及交接记录；到期未还触发提醒和逾期状态，不自动虚构归还。 |
| FR-110 | 创建盘点任务并扫码盘点 | 资产管理员 | 按部门、位置或分类创建盘点批次；扫码区分已盘、重复、非本批次和状态异常，结束后输出盘盈、盘亏和待核查清单。 |
| FR-111 | 库存预警 | 资产管理员 | 按分类配置安全可用数量；低于阈值时在Web看板和待办中预警，并可下钻到占用、维修和报废明细。 |
| FR-112 | 维保计划与提醒 | 资产管理员 | 按日期、使用时长或次数创建维保计划；到期提醒、登记送修和完工结果，维修期间禁止借用。 |
| FR-113 | 折旧规则与月度计算 | 资产管理员 | 配置折旧方法、年限、残值率和启用日期；按月形成原值、累计折旧和净值快照，规则变更版本化且可追溯。 |
| FR-114 | 报废与处置审批 | 资产管理员 | 录入原因、鉴定、残值和附件，完成申请、审批、实物处置确认；已处置资产不可恢复借用，历史数据保留。 |
| FR-115 | 资产报表分析与受控导出 | 资产管理员、经营管理层 | 展示数量、原值、净值、利用率、借用频次、逾期、维保成本、损坏和盘点差异；导出遵守数据权限并记录审计。 |
| FR-116 | 新媒体拍摄档期日历 | 新媒体部门员工、部门负责人 | 提供周/月看板，展示主题、项目、时间、地点、人员、状态和器材准备情况；支持按项目、人员、类型与资源状态筛选。 |
| FR-117 | 拍摄档期关联器材 | 新媒体部门员工、资产管理员 | 档期可选择具体资产或器材类别并生成借用申请；档期详情可下钻资产和借用单，资产详情可反查使用档期。 |
| FR-118 | 人员与器材占用冲突校验 | 新媒体部门员工、部门负责人、资产管理员 | 创建、变更和审批档期时同时校验人员与设备的重叠占用；冲突未处理不得确认，取消档期释放未出库预占并保留记录。 |

上述功能统一记录来源、操作者、发生时间、前后状态、关联对象与版本。无权限、重复提交、并发占用、扫码对象不一致、设备异常或计算失败时，系统必须说明原因和可执行的下一步，不得静默失败。
...
交互规格覆盖PC端完整管理和移动端高频处理，共37个页面、12条核心路径，映射FR-001至FR-118。统一工作台按角色显示菜单、待办、预警和快捷入口；详情页必须提供当前状态、可执行动作、关联记录和操作历史。

### 7.2 原型入口与版本

高保真原型位于 `高保真原型/index.html`。本次变更已增加固定资产Web管理、移动扫码演示和新媒体拍摄档期日历；原型用于评审信息架构、状态、操作入口和两条业务闭环，最终字段、审批人配置与折旧会计口径仍以实施配置为准。
...
| PAGE-004 | 管理层待分配线索池 | 统一工作台 | 董事会/经营管理层 | 客户、线索、线索跟进、联系人、线索分配批次 | FR-009、FR-010、FR-014、FR-015、FR-019、FR-020、FR-016 |
| PAGE-005 | 线索详情与跟进 | 统一工作台 | 董事会/经营管理层、线索责任人、商务责任人 | 客户、线索、线索跟进、联系人 | FR-017、FR-018、FR-011、FR-021 |
| PAGE-006 | 线索池权限与分配规则 | 统一工作台 | 董事会/经营管理层、系统管理员 | 客户、线索、线索跟进、联系人、线索分配规则 | FR-012、FR-013 |
...
| PAGE-032 | 固定资产总览与台账 | Web管理后台 | 资产管理员、经营管理层、普通员工 | 固定资产、资产分类、资产二维码、资产使用记录 | FR-101、FR-102、FR-103、FR-104、FR-111 |
| PAGE-033 | 资产借用归还与审批 | Web管理后台 | 资产管理员、普通员工 | 资产申请、审批记录、资产占用、资产使用记录 | FR-105、FR-106、FR-109 |
| PAGE-034 | 移动扫码领用归还与盘点 | 移动端 | 资产管理员、普通员工 | 资产二维码、资产申请、归还验收、盘点任务、盘点明细 | FR-107、FR-108、FR-110 |
| PAGE-035 | 资产维保折旧与处置 | Web管理后台 | 资产管理员 | 维保计划、维保记录、折旧规则、折旧快照、处置申请 | FR-112、FR-113、FR-114 |
| PAGE-036 | 固定资产报表 | Web管理后台 | 资产管理员、经营管理层 | 资产统计、使用统计、盘点差异、维保成本、折旧快照 | FR-115 |
| PAGE-037 | 新媒体拍摄档期日历 | 统一工作台 | 新媒体部门员工、部门负责人、资产管理员 | 拍摄档期、档期参与人、资产占用、资产申请、固定资产 | FR-116、FR-117、FR-118 |
...
| PATH-002 | 线索录入、管理层分配到跟进 | 董事会/经营管理层、线索责任人 | PAGE-004 → PAGE-005 | FR-009、FR-010、FR-014、FR-015、FR-016、FR-017、FR-018、FR-019、FR-020 |
...
| PATH-011 | 固定资产全生命周期 | 资产管理员、普通员工 | PAGE-032 → PAGE-033 → PAGE-034 → PAGE-035 → PAGE-036 | FR-101至FR-115 |
| PATH-012 | 拍摄档期与器材协同 | 新媒体部门员工、部门负责人、资产管理员 | PAGE-037 → PAGE-033 → PAGE-034 → PAGE-037 | FR-116、FR-117、FR-118、FR-105至FR-108 |
...
- 移动端优先支持领取/跟进线索、处理待办、更新任务、提交日报、确认会议、查看工资条、知识检索以及固定资产扫码盘点、领用和归还；复杂配置在PC端完成。
...
- 普通员工可查看允许借用的资产和本人申请/使用记录，不可修改资产原值、折旧、盘点结论、维保结论和处置状态；资产管理员可管理资产，但不得代替员工确认实际领用事实。
...
| 固定资产 | 固定资产、资产分类、资产二维码、资产申请、资产占用、资产使用记录、盘点任务、盘点明细、维保计划、维保记录、折旧规则、折旧快照、处置申请 |
| 拍摄协同 | 拍摄档期、档期参与人、档期资产需求、资产占用、关联借用申请 |
...
- 固定资产编号和当前有效二维码均唯一；资产状态、时间区间占用和审批状态必须一致，任何资产在同一有效时间区间仅允许一个生效占用。
- 折旧快照保存规则版本、计算周期、原值、残值、当期折旧、累计折旧和净值；历史快照不可被新规则覆盖。
- 拍摄档期、资产申请、资产占用与使用记录通过稳定ID关联；取消档期只释放尚未实际领用的预占，不删除审批和使用历史。
...
1. 能从线索来源追踪到管理层分配、负责人跟进、商机、合同、收款、项目、交付和复盘，关键节点操作记录完整。
2. 全员录入的有效线索统一进入管理层待分配池，普通员工不可见；管理层可批量勾选、选择具体人员并按默认平均分配或逐条指定完成分配。
...
10. 固定资产可完成登记、二维码绑定、申请、审批、扫码领用、扫码归还、盘点、维保、折旧、处置和报表闭环，任一步均可追溯。
11. 新媒体拍摄档期可在周/月日历中查看并关联具体器材；人员或设备时间冲突会被明确阻止，档期与借用记录可双向下钻。

### 12.2 功能与交互验收

- FR-001至FR-118逐项通过“正常、空数据、无权限、规则不满足、状态冲突、外部失败”适用场景验证。
- 37个页面均能找到承载FR的入口、数据区块、操作、结果和历史；12条核心路径可从起点完成至业务结论。
- 必填、格式、唯一性和跨字段校验清晰；失败不清空已填内容；重复提交不产生重复数据。
- PC端完成全部管理功能，移动端完成规定高频任务及资产扫码盘点/领用/归还，消息深链进入正确页面并再次鉴权。
...
- 118项功能验收均有结果，严重级缺陷清零；一般缺陷有责任人和修复计划。
...
| R-05 | 高保真原型持续迭代 | 当前原型已覆盖固定资产与拍摄档期新增闭环 | 原型覆盖FR-001至FR-118并完成评审 |
| R-06 | 折旧会计口径、资产分类安全库存和维保周期尚未定版 | 先提供版本化、可配置框架，原型使用演示值 | 资产管理责任部门与财务确认正式规则并发布配置版本 |



## 14 来源产物清单

以下文件用于需求追溯；正式需求基线以本PRD正文为准。
