# CRM+OKR公司经营管理全域（对象关系分析） - 业务实体分析

## 1. 基本信息

- **业务域名称**: CRM+OKR公司经营管理全域（对象关系分析）
- **业务域描述**: 基于已验证的44个业务对象，分析单一公司主系统、线索池子系统和AI知识库子系统中的组合、聚合、关联、依赖、自关联及关系基数。
- **分析时间**: 2026-07-21

## 2. 业务实体概述

### 2.1 业务实体定义
本阶段沿用B2.2已确认业务对象，并用关系语义说明对象如何共同完成业务：组合表示从属记录依赖主对象存在，聚合表示归属但可独立识别，关联表示业务参与，依赖表示下游使用上游结果，自关联表示同类对象间的层级或前后依赖。

### 2.2 业务实体与数据库表的区别
对象关系表达业务归属、责任、基数和约束，不代表数据库外键、连接表、索引、接口字段或ORM关联。

### 2.3 业务实体分析的业务价值
为生命周期、对象行为、权限和系统功能提供清晰责任边界；三类项目交付与两类会议作为项目类型和会议类型表达，不重复建立泛化子对象。
...
|-------|---------|---------|---------|---------|
| 部门 | 岗位 | 一对多 | [聚合] 部门设置多个岗位。 | 岗位须处于有效组织范围。 |
| 岗位 | 员工 | 一对多 | [关联] 岗位可由多名员工任职。 | 员工当前岗位随入转离生效。 |
| 员工 | 劳动合同 | 一对多 | [组合] 员工可有连续的劳动合同版本。 | 劳动合同依赖员工劳动关系，历史合同不得覆盖。 |
| 员工 | 员工变动 | 一对多 | [组合] 员工产生入职、转岗和离职记录。 | 员工变动依赖员工存在；按生效时间改变当前责任且历史不分离。 |
| 员工 | 考勤记录 | 一对多 | [组合] 员工按工作日形成考勤事实。 | 考勤记录依赖员工，异常需确认后才进入工资相关处理。 |
| 员工 | 每日工作记录 | 一对多 | [组合] 员工按日期记录每日工作。 | 每日工作记录依赖员工，首期仅今日工作、明日计划和产出引用。 |
| 客户 | 联系人 | 一对多 | [聚合] 一个客户主体可有多个联系人。 | 同公司不同联系人分别保留；联系人可在误合并撤销后恢复原客户关系。 |
| 客户 | 线索 | 一对多 | [聚合] 一个客户可对应多次独立商机。 | 同客户不同联系人不自动合并线索，线索保持独立生命周期。 |
| 线索 | 线索跟进 | 一对多 | [组合] 线索包含连续跟进事实。 | 跟进记录依赖所属线索，历史跟进和操作记录必须保留。 |
| 客户 | 客户合同 | 一对多 | [聚合] 客户可签署多个业务合同。 | 合同独立记录范围和付款条件。 |
| 客户合同 | 应收计划 | 一对多 | [组合] 合同形成一个或多个应收节点。 | 应收计划依赖合同付款条件。 |
| 应收计划 | 收款记录 | 一对多 | [关联] 一个应收节点可由多笔收款满足。 | 仅核验通过金额计入实收。 |
| 客户合同 | 开票记录 | 一对多 | [组合] 合同可产生多次开票处理。 | 开票记录依赖合同，作废和红冲保留历史。 |
| 客户合同 | 退款记录 | 一对多 | [组合] 合同可产生退款事项。 | 退款记录依赖合同，须有原因和处理结论。 |
| 客户合同 | 项目 | 一对多 | [依赖] 一个合同可为一个或多个交付项目提供成交范围。 | 项目引用明确合同和交付范围，正式启动还依赖财务条件核验。 |
| 项目 | 项目财务汇总 | 一对一 | [组合] 每个项目形成一个当前财务汇总。 | 项目财务汇总依赖项目存在，随有效财务记录重算。 |
| 项目 | 项目角色分配 | 一对多 | [组合] 项目由多个角色分配组成。 | 角色分配依赖所属项目，同一时点项目负责人唯一。 |
| 员工 | 项目角色分配 | 一对多 | [关联] 员工可在多个项目承担角色。 | 角色分配须记录有效期间。 |
| 项目 | 项目里程碑 | 一对多 | [组合] 项目基线包含多个里程碑。 | 里程碑依赖所属项目，基线变更保留原计划。 |
| 项目 | 项目任务 | 一对多 | [组合] 项目分解为可执行任务。 | 任务依赖所属项目，正式任务明确责任和期限。 |
| 项目任务 | 交付成果 | 一对多 | [组合] 任务可提交多个成果或版本。 | 成果保留所属任务和版本，不得覆盖历史。 |
| 项目 | 验收记录 | 一对多 | [组合] 项目可经历多次验收和整改。 | 验收记录依赖所属项目，结项前须形成明确验收结论。 |
| 项目 | 工单 | 一对多 | [聚合] 项目可关联跨部门协作工单。 | 非项目工单可以不关联项目。 |
| 工单 | 工单流转记录 | 一对多 | [组合] 工单由连续流转动作构成。 | 流转记录依赖所属工单，转派、升级、关闭和重开均留痕。 |
| 项目 | 排期 | 一对多 | [聚合] 项目可形成多个人员活动排期。 | 培训、沙龙等非项目排期允许不关联项目。 |
| 排期 | 排期参与人 | 一对多 | [组合] 排期包含一个或多个参与人。 | 参与关系依赖所属排期，讲师档期须独立确认。 |
| 员工 | 排期参与人 | 一对多 | [关联] 员工可参加多个排期。 | 确认前检查时间冲突和负荷。 |
| 排期 | 会议 | 一对一 | [关联] 内部培训或外部沙龙以排期承载时间占用。 | 会议必须关联排期，不创建会议室对象。 |
| 排期 | 日历同步记录 | 一对多 | [组合] 排期创建、变更和取消形成同步尝试。 | 同步记录依赖所属排期，系统排期是业务主记录。 |
| 员工 | 工时记录 | 一对多 | [关联] 员工记录多个业务事项的投入。 | 工时须关联实际业务事项。 |
| 项目任务 | 工时记录 | 一对多 | [关联] 任务可由多个员工分次记录工时。 | 计划与实际工时分开保留。 |
| 考核周期 | 绩效规则 | 一对多 | [关联] 周期引用一个或多个岗位规则版本。 | 评分须使用当期生效版本。 |
| 考核周期 | 目标 | 一对多 | [组合] 月度周期包含公司、部门和个人目标。 | 目标按确认的逐级对齐规则建立。 |
| 目标 | KPI任务 | 一对多 | [聚合] 个人考核可关联多个KPI考核项。 | KPI口径和权重来自生效规则。 |
| 员工 | 绩效结果 | 一对多 | [聚合] 员工每个考核周期形成绩效结果。 | 同一员工同一周期仅一个当前锁定结果。 |
| 绩效结果 | 绩效证据 | 一对多 | [聚合] 绩效结果由多项业务证据支撑。 | 同一成果不得重复计分。 |
| 绩效结果 | 绩效申诉 | 一对多 | [组合] 绩效结果可产生申诉及复核记录。 | 申诉记录依赖被申诉结果，结案前不得锁定或进入工资。 |
| 绩效结果 | 工资核算记录 | 一对一 | [依赖] 锁定绩效结果进入当期工资核算。 | 工资核算只读取已锁定结果，固定工资不受绩效系数影响。 |
| 工资核算记录 | 工资条 | 一对多 | [组合] 一次核算可形成初版和更正版工资条。 | 工资条依赖核算记录，仅本人可见且原版保留。 |
| 知识来源 | 知识内容 | 一对多 | [聚合] 授权来源沉淀多个知识内容。 | 内容保持来源引用，查看范围不得超过来源授权。 |
| 知识内容 | 知识版本 | 一对多 | [组合] 知识内容随整理和纠正形成多个版本。 | 版本依赖所属知识内容，检索结果展示来源、版本和更新时间。 |
| 知识内容 | 知识反馈 | 一对多 | [组合] 知识内容可收到多次治理反馈。 | 反馈关联具体内容，处理形成纠正、失效或下架结论。 |
| 员工 | 审计记录 | 一对多 | [关联] 员工关键操作形成审计记录。 | 审计可见范围留待权限阶段确定。 |
| 部门 | 员工 | 一对多 | [聚合] 部门管理当前归属员工。 | 员工转岗按生效时间改变当前部门，历史归属保留。 |
| 部门 | 目标 | 一对多 | [关联] 部门承接公司目标并形成部门目标。 | 部门目标必须对齐公司目标。 |
| 岗位 | 绩效规则 | 一对多 | [关联] 岗位在不同周期适用绩效规则版本。 | 当期评分引用生效版本，历史版本不得覆盖。 |
| 联系人 | 线索 | 一对多 | [关联] 联系人可参与多个独立商机。 | 同客户不同联系人分别保留线索；同联系人也允许不同商机。 |
| 员工 | 线索 | 一对多 | [关联] 员工可作为多条线索的当前主负责人。 | 单条线索当前主负责人唯一；员工保护期线索最多100条。 |
| 项目里程碑 | 项目任务 | 一对多 | [聚合] 里程碑汇总为其达成服务的任务。 | 任务可在项目基线内归属一个里程碑。 |
| 项目任务 | 项目任务 | 多对多 | [自关联] 项目任务可依赖其他任务完成。 | 依赖关系不得形成无法执行的循环；具体校验在对象行为阶段定义。 |
| 交付成果 | 验收记录 | 一对多 | [关联] 一个交付成果可经历提交、整改和再次验收。 | 验收记录明确对应成果范围和版本。 |
| 员工 | 工单 | 多对多 | [关联] 员工可作为发起人、处理人或管理责任人参与工单。 | 具体责任变化由工单流转记录保存。 |
| 工单 | 工时记录 | 一对多 | [关联] 工单处理可形成员工实际工时。 | 工时须同时关联员工和工单。 |
| 排期 | 工时记录 | 一对多 | [关联] 已完成活动排期可形成实际工时。 | 实际工时由参与员工确认，不以计划占用自动代替。 |
| 考核周期 | 绩效结果 | 一对多 | [组合] 月度考核周期汇总员工绩效结果。 | 同一员工同一周期只有一个当前结果。 |
| 绩效规则 | 绩效结果 | 一对多 | [依赖] 多个绩效结果可引用同一生效规则版本。 | 结果必须保存使用的规则版本。 |
| 目标 | 目标 | 一对多 | [自关联] 公司目标向部门目标、部门目标向个人目标建立对齐关系。 | 公司到部门强制对齐；个人至少一项目标对齐部门。 |
| 目标 | 绩效证据 | 一对多 | [关联] 目标进展和结果由业务证据支撑。 | 同一成果不得重复计分。 |
| KPI任务 | 绩效证据 | 一对多 | [关联] KPI结果引用可核验业务证据。 | 证据口径须符合当期绩效规则。 |
| 员工 | 工资核算记录 | 一对多 | [关联] 员工每个工资周期形成核算记录。 | 核算须引用本人当期锁定绩效结果。 |
| 员工 | 知识反馈 | 一对多 | [关联] 员工可对授权可见知识提出反馈。 | 反馈不得扩展员工原有知识查看范围。 |
| 员工 | 客户 | 一对多 | [关联] 员工可作为多个客户的当前主负责人。 | 同一客户当前主负责人唯一，具体可见范围在C3确认。 |
| 员工 | 线索跟进 | 一对多 | [关联] 员工对本人负责线索形成跟进记录。 | 非当前负责人不得修改跟进事实。 |
| 线索 | 客户合同 | 一对多 | [依赖] 成交线索为后续客户合同提供来源和承诺信息。 | 合同独立审核生效，不能仅因线索成交自动生效。 |
| 客户 | 项目 | 一对多 | [聚合] 客户可对应多个交付项目。 | 项目同时须引用明确合同和交付范围。 |
| 客户合同 | 收款记录 | 一对多 | [聚合] 合同可对应多笔实际收款。 | 收款可进一步分配至一个或多个应收节点，仅核验通过金额计入实收。 |
| 员工 | 项目任务 | 多对多 | [关联] 员工可负责或参与多个项目任务。 | 任务责任和参与历史不得因人员变动丢失。 |
| 员工 | 工单流转记录 | 一对多 | [关联] 员工执行工单分派、受理、转派、确认等动作。 | 每次流转保存动作人、时间和结果。 |
| 会议 | 日历同步记录 | 一对多 | [关联] 会议创建、变更和取消对应飞书同步结果。 | 通过所属排期更新原飞书事件，不重复创建。 |
| 员工 | 目标 | 一对多 | [关联] 员工在周期内承担个人目标。 | 个人至少一项目标对齐部门目标。 |
| 员工 | KPI任务 | 一对多 | [关联] 员工按岗位承担KPI考核项。 | KPI口径和权重引用当期规则。 |
| 考核周期 | KPI任务 | 一对多 | [组合] 周期包含当期KPI考核项。 | 跨周期不得复用已结束考核项结果。 |
| 员工 | 绩效申诉 | 一对多 | [关联] 员工可对本人绩效结果发起申诉。 | 申诉只针对本人当期结果并在锁定前结案。 |
| 考核周期 | 工资核算记录 | 一对多 | [组合] 月度周期汇总员工工资核算记录。 | 核算须使用同周期已锁定绩效结果。 |
| 员工 | 工资条 | 一对多 | [关联] 员工可查看本人各周期工资条及更正版本。 | 工资条不得向无权人员展示。 |
| 员工 | 知识来源 | 一对多 | [关联] 员工可作为知识来源或内容责任人。 | 责任人不等于自动获得超出授权范围的查看权限。 |
